...
if [ -z "${MAVEN_SETTINGS_SECURITY_FILE}" ]; then
    MAVEN_SETTINGS_SECURITY_FILE=${CI_CACHE}/settings-security.xml
fi
...
if [ ! -f "${DEPLOY_LOCAL_REPO_IF_NEED}" ]; then
    mkdir -p ${DEPLOY_LOCAL_REPO_IF_NEED}
fi
